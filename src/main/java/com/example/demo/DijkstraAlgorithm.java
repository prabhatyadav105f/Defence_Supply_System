...
public class DijkstraAlgorithm {
    ...
    private final Set<String> blockedRoutes; // store blocked edges like "A->B"

    public DijkstraAlgorithm(Map<String, Map<String, Double>> graph) {
        this.graph = graph;
        this.blockedRoutes = new HashSet<>();
    }

    // ✅ Allow external code to block routes
    public void blockRoute(String from, String to) {
        blockedRoutes.add(from + "->" + to);
    }
    ...
    public List<Map<String, Object>> findShortestPathWithSteps(String start, String end) {
        ...
        while (!pq.isEmpty()) {
            ...
            for (Map.Entry<String, Double> neighbor : graph.get(currentVertex).entrySet()) {
                ...
                // ✅ Skip blocked route
                String routeKey = currentVertex + "->" + neighborVertex;
                if (blockedRoutes.contains(routeKey)) {
                    steps.add(createStep("skip", currentVertex, neighborVertex,
                ...
                if (alt < distances.get(neighborVertex)) {
                    ...
                    // Add update step
                    steps.add(createStep("update", currentVertex, neighborVertex,
                                         new HashMap<>(distances),
                                         new HashMap<>(previousNodes)));
                }
            }
    ...
    private Map<String, Object> createStep(String type, String current, String neighbor,
